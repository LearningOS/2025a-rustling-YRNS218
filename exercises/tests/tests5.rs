...
// When `unsafe` is marked on a code block enclosed by curly braceses,
...
unsafe fn modify_by_address(address: usize) {
    // SAFETY: The function's contract guarantees that `address` is a valid mutable pointer to a `u32`,
    // so converting it to a `*mut u32` and dereferencing is safe.
    unsafe {
        *(&address as *const usize as *mut u32) = 0xAABBCCDD;
    }
}
